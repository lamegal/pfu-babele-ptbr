### Project Fabula Ultima - Localização para Português Brasileiro
![Fabula Ultima Mod](https://need.games/wp-content/uploads/2024/06/FU-MOD-bgW-300x121.png)

## Prefácio
Há algum tempo, tenho me dedicado a traduzir os termos e itens dos diversos compêndios do nosso maravilhoso sistema Fabula Ultima dentro do Project FU para FoundryVTT. Este módulo tem como objetivo traduzir os títulos, termos, descrições e demais características dos compêndios disponíveis no sistema de acordo com a Licença Aberta e com as traduções oficiais realizadas pela Jambo Editora no Brasil.
##### Esta localização é uma produção independente deste autor e não possui qualquer vínculo com a Need Games, Rooster Games ou Jambo Editora. As regras do jogo fornecidas no compêndio do sistema seguem e são publicadas sob a LICENÇA ABERTA DE FABULA ULTIMA TTRPG v1.0. É possível ler todo o conteúdo desta licença através da aquisição gratuita deste item no site da Jambo Editora (https://jamboeditora.com.br/produto/fabula-ultima-recompensas-digitais-2).
##### Segui as diretrizes da RoosterEma e da editora-chefe da Jambo, Karen Soarele, garantindo que o livro básico e seus suplementos permaneçam parte integrante da sua experiência. Para utilizar este sistema em sua totalidade, você precisará do Livro de Regras Básico de Fabula Ultima.

##### O código-fonte deste repositório está sujeito aos termos da Licença MIT. Para mais detalhes, consulte o arquivo LICENÇA incluído neste repositório.

##### Fabula Ultima é um jogo de RPG criado por Emanuele Galletto e publicado pela Need Games. Fabula Ultima é © Need Games e Rooster Games. A Jambo Editora é a responsável pela publicação deste conteúdo no território brasileiro. Para adquirir, visite o site da editora.

## Instalação manual
Para instalar o módulo no Foundry, são necessários alguns passos simples:
1. Abra a página de configuração da sua instância do FoundryVTT.
2. Mude para a aba Módulos Adicionais.
3. Clique em Instalar Módulo.
4. Insira este URL no campo URL do Manifesto: https://github.com/lamegal/pfu-babele-ptbr/releases/latest/download/module.json
5. Clique em Instalar.

## **Aviso**

### Para que este módulo funcione, o módulo Babele deve estar instalado.

## Compêndios traduzidos
Estes são os compêndios que traduzi até agora. Tentarei formular as limitações em subitens.

#### **Criação de Personagem**
##### - Classes
##### - Habilidades (Será modificado para Poderes em uma próxima atualização, de acordo com os novos termos da Jambo Editora).
##### - Feitiços
##### - Habilidades Heroicas  (Será modificado para Poderes Heroicos em uma próxima atualização, de acordo com os novos termos da Jambo Editora).

#### **Equipamento**
##### - Equipamento Básico
##### - Consumíveis
##### - Tecnosferas (Ainda não traduzido)
##### - Hoplosferas (Ainda não traduzido)
##### - Módulos de Veículo

#### **Ferramentas do Mestre**
##### - Narrador 
##### >> Feitiços de NPC traduzidos
##### - Tabelas
##### >> Alquimia (Alvo) e Alquimia (Efeito) Traduzidos. Ainda faltam: Efeito de Iguaria, Aposta, Sabor do Ingrediente e Hoploesferas Aleatórias.
  
#### **Diários (Journals)**
#####  Nenhuma dos Handouts de Regras foi traduzido ainda. Isso será feito em uma atualização posterior.
